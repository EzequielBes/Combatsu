...
O objetivo não é produzir um jogo completo agora, e sim um protótipo
pequeno e testável (vertical slice) que permita avaliar, jogando, se o
"feel" de mover, bater e usar objetos do cenário como arma é divertido o
suficiente para justificar investir tempo nos sub-projetos seguintes.
Não há prazo de lançamento definido — o desenvolvimento é exploratório e
incremental.

**Critério de sucesso:** existe uma cena jogável com o personagem se
...
- **Motor:** Unity.
- **Dimensão:** o jogo é modelado e simulado em **3D** (física 3D,
  personagens e objetos como meshes 3D com Rigidbody/Collider 3D), mas
  jogado com câmera lateral fixa, como um metroidvania 2D.
- **Plano de movimento:** eixo de profundidade **travado**. Personagem e a
  maioria dos objetos só se movem nos eixos X (horizontal) e Y (vertical).
  Na prática, joga-se como um jogo 2D com modelos e física 3D por baixo.
  Essa escolha prioriza legibilidade de plataforma e simplicidade de
  design de fases sobre volume/profundidade visual.
...
- Baseado em `CharacterController` (ou controlador cinemático próprio com
  raycasts), **não** em física livre — mesmo o jogo sendo 3D por baixo, o
  personagem precisa da precisão determinística de um controlador de
  plataforma (pulos consistentes, sem deslizar em rampas/colisões, sem
  ricochete). Esse é o padrão em metroidvanias de precisão.
- Movimento travado ao eixo de profundidade da cena (ver "Plano de
  movimento" acima).
...
Cada objeto do cenário (cadeira, garrafa, etc.) é dirigido por um asset de
dados (`ScriptableObject`) com propriedades como: peso, dano causado,
durabilidade (nº de impactos até quebrar), força de arremesso, e tags
(ex.: "cortante", "inflamável" — reservado para uso futuro por técnicas
amaldiçoadas). Criar um objeto novo é preencher esses dados, não escrever
código novo.
...
- **Repouso** — objeto solto no cenário. Colide fisicamente só com o
  terreno/ambiente; ignora completamente a colisão física com as camadas
  de Player e Inimigo (eles podem encostar/atravessar sem ser bloqueados).
  A coleta acontece por uma zona de interação por proximidade (trigger),
  não por colisão física.
...
- **Em uso** — golpe com o objeto na mão, ou objeto em voo após ser
  arremessado. Vira uma hitbox ativa. Cada acerto carrega uma referência
  de **dono** (quem bateu/arremessou); ao colidir com um personagem, se
  esse personagem for o dono, o hit é ignorado — isso evita que o player
  se acerte com o próprio arremesso mesmo que player e inimigo estejam na
  mesma categoria de "alvo atingível". Bater ou arremessar um objeto conta
  sempre como golpe **forte** (ver Sistema de impacto).
...
- **Retorno a Repouso** — se um objeto resistente é arremessado, atinge
  ...
  Repouso (perde a hitbox ativa, volta a ignorar Player/Inimigo) até ser
  pego de novo.

### 3. Sistema de combate corpo a corpo

Combo desarmado simples de 2-3 hits (ex.: soco, soco, chute), implementado
com hitbox/hurtbox (colisores trigger) — não colisão física direta, para
manter o combate responsivo e previsível. O último hit do combo é
...
- **Golpe leve** (hits normais de combo): reação por animação apenas
  (hit-reaction clips). Sem física. Mantém o combate legível e responsivo.
- **Golpe forte** (finalizador de combo, uso de objeto do cenário — e no
  ...
  no esqueleto do inimigo (tronco, braços, pernas, cabeça viram
  `Rigidbody` + `ConfigurableJoint`), com um impulso aplicado na direção
  do golpe. Não é um "active ragdoll" full-time controlando a locomoção —
  é o padrão clássico de alternar entre animação e ragdoll temporário.
...
Essa cena é o que será jogado para decidir se vale a pena avançar para os
sub-projetos 2 e 3.

## Riscos e pontos de atenção para o plano de implementação

- Ajuste fino de física (physics materials, evitar jitter/instabilidade)
  para os objetos em Repouso e durante o ragdoll — validar cedo com testes
  manuais, não só em teoria.
- A transição de estado do objeto (Repouso ↔ Segurando ↔ Em uso) precisa
  trocar corretamente `isKinematic`/camadas de colisão em cada mudança de
  estado, incluindo casos de borda (ex.: objeto largado no meio do ar,
  personagem morre segurando um objeto).
- A transição ragdoll → animação ("levantar") é conhecida por ser visualmente
  ingrata se feita de forma simplista; vale reservar tempo de iteração
  visual, não só funcional.
